...
public enum StorageComponent {
    ...
    SPARK2 {
        ...
        public Set<StorageLocationV4Request> getLocations(TestStorageLocation tsl) {
            return Set.of(
                    createStorageLocationRequest(
                            String.format("%s//%s/%s/oplogs/spark2-history", tsl.getStorageTypePrefix(), tsl.getClusterName(), tsl.getPathInfix()),
                            "spark2-defaults",
                            "spark.eventLog.dir"),
                    createStorageLocationRequest(
                            String.format("%s//%s/%s/oplogs/spark2-history", tsl.getStorageTypePrefix(), tsl.getClusterName(), tsl.getPathInfix()),
    ...
    YARN {
        ...
        public Set<StorageLocationV4Request> getLocations(TestStorageLocation tsl) {
            return Set.of(
                    createStorageLocationRequest(
                            String.format("%s//%s/%s/oplogs/yarn-app-logs", tsl.getStorageTypePrefix(), tsl.getClusterName(), tsl.getPathInfix()),
                            "yarn-site",
